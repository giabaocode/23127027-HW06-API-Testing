...
- **Oracle Classification:** **`SPECIFIED (Semantic Acceptance) / INFERRED (HTTP Status)`**
...
#### Expected Result
- **Expected Semantic Behavior:** Server safely handles empty JSON object with controlled response; no server crash and zero unsafe cart mutation
...
#### Expected Result
- **Expected Semantic Behavior:** Server handles extra properties safely without crashing process; extra unrecognized fields must not cause unintended state side-effects
