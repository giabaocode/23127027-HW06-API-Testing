...
| **`COV-FR12-03`** | `/api/admin/users` | `GET` | Grant administrative listing | Admin User (`role: 'admin'`) | `FR-12` | AUTHORIZED (Not Blocked by SEC-02/03) | `200 OK` (Inferred from SUT) | Returns array containing system users | Login seeded admin | Positive Functional | Verifies admin capability |
...
| **`COV-FR12-05`** | `/api/admin/users/:id` | `DELETE` | Grant user deletion to admin | Admin User (`role: 'admin'`) | `FR-12` | AUTHORIZED (Not Blocked by SEC-02/03) | `200 OK` (Inferred from SUT) | Target user deleted from database | Create disposable target user | Positive Mutation | Operates on disposable user |
...
| **`COV-FR12-08`** | `/api/admin/orders` | `GET` | Grant full order history to admin | Admin User (`role: 'admin'`) | `FR-12` | AUTHORIZED (Not Blocked by SEC-02/03) | `200 OK` (Inferred from SUT) | Returns system-wide order list | Admin token | Positive Functional | Verifies admin capability |
...
| **`COV-FR12-10`** | `/api/admin/orders/:id/status` | `PUT` | Grant order status update to admin | Admin User (`role: 'admin'`) | `FR-12` | AUTHORIZED (Not Blocked by SEC-02/03) | `200 OK` (Inferred from SUT) | Order status updated to `confirmed` | Reference existing order | Positive Mutation | Verifies admin capability |
...
| **`COV-FR12-13`** | `/api/admin/import-products` | `POST` | Grant catalog import to admin | Admin User (`role: 'admin'`) | `FR-12` | AUTHORIZED (Not Blocked by SEC-02/03) | `200 OK` (Inferred from SUT) | Bulk products imported into catalog | Admin token + valid CSV payload | Positive Mutation | Verifies admin capability |
...
| **`COV-FR12-16`** | `/api/coupons` | `GET` | Grant coupon overview to admin | Admin User (`role: 'admin'`) | `FR-12` | AUTHORIZED (Not Blocked by SEC-02/03) | `200 OK` (Inferred from SUT) | Returns list of coupons | Admin token | Positive Functional | Verifies admin capability |
...
| **`COV-FR12-19`** | `/api/admin/coupons` | `POST` | Grant coupon creation to admin | Admin User (`role: 'admin'`) | `FR-12` | AUTHORIZED (Not Blocked by SEC-02/03) | `200 OK` (Inferred from SUT) | Coupon successfully created | Admin token + unique code | Positive Mutation | Verifies admin capability |
...
| **`COV-FR12-23`** | `/api/products` | `POST` | Grant product creation to admin | Admin User (`role: 'admin'`) | `FR-12` | AUTHORIZED (Not Blocked by SEC-02/03) | `200 OK` (Inferred from SUT) | Product created with new ID | Admin token + product body | Positive Mutation | Verifies admin capability |
...
| **`COV-FR12-26`** | `/api/products/:id` | `PUT` | Grant product update to admin | Admin User (`role: 'admin'`) | `FR-12` | AUTHORIZED (Not Blocked by SEC-02/03) | `200 OK` (Inferred from SUT) | Target product updated | Admin token on disposable item | Positive Mutation | Verifies admin capability |
...
| **`COV-FR12-31`** | `/api/categories` | `POST` | Grant category creation to admin | Admin User (`role: 'admin'`) | `FR-12` | AUTHORIZED (Not Blocked by SEC-02/03) | `200 OK` (Inferred from SUT) | Category created | Admin token + category body | Positive Mutation | Verifies admin capability |
...
| **`COV-FR12-34`** | `/api/categories/:id` | `DELETE` | Grant category deletion to admin | Admin User (`role: 'admin'`) | `FR-12` | AUTHORIZED (Not Blocked by SEC-02/03) | `200 OK` (Inferred from SUT) | Category deleted | Admin token on disposable item | Positive Mutation | Verifies admin capability |
